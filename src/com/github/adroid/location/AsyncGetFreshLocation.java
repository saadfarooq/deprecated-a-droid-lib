...
public class AsyncGetFreshLocation extends AsyncGetLocation {
	
	private final static String TAG = "AsyncGetFreshLocation";
	...
	public AsyncGetFreshLocation() {};
	
	public AsyncGetFreshLocation(int timeInHours) {
		this.timeInHours = timeInHours;
	}
	
	public AsyncGetFreshLocation(AsyncCaller caller) {
		this.caller = caller;
	}
	
	public AsyncGetFreshLocation(AsyncCaller caller, int requestCode) {
		this.caller = caller;
		this.requestCode = requestCode;
	}
	
	public AsyncGetFreshLocation(AsyncCaller caller, int requestCode, int timeInHours) {
		this.caller = caller;
		this.requestCode = requestCode;
		this.timeInHours = timeInHours;
	}
	...
	protected void onPreExecute() {
		...
		for (String provider : providers) {
			...
			if (mLocation.getTime() > (System.currentTimeMillis()- (timeInHours * 60 * 60 * 1000) ) ) {
				return;
			}
		}
		mLocation = null;
	...
	protected void onPostExecute(Location result) {
		if ( caller != null ) ((AsyncCaller) caller).onBackgroundTaskCompleted(requestCode, result);
	}
